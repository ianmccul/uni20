...
class Matrix {
 ...
 public:
  struct Header {  // POD — safe to send via MPI as raw bytes
    int id = -1;
    int node_id = -1;
    size_t dim1 = 0;
    size_t dim2 = 0;
  ...
  explicit Matrix(Header h)
      : impl(std::make_shared<MatrixImpl>(
            MatrixImpl{nullptr, h.id, h.node_id, h.dim1, h.dim2})) {}

  Header toHeader() const {
    return {impl->id, impl->node_id, impl->dim1, impl->dim2};
  }
...
namespace std {
template <>
struct hash<tensor::Matrix> {
  size_t operator()(const tensor::Matrix &m) const {
