...
[`libraries.md`](libraries.md): that document describes
...
runtime fallback discussion here uses the `kernel_accepts_types(...)` /
`try_kernel(...)` split from [`../../architecture/kernel_dispatch.md`](../../architecture/kernel_dispatch.md).
...
- `kernel_accepts_types(...)` checks type-level facts, such as whether operands are
