...
class MatrixAllocator {
 private:
   ...
   */
  std::vector<Matrix> initializeMatricesFromChunk(
      const std::vector<std::pair<int, int>>& dims, double* chunkPtr,
      bool shouldInitialize = false, bool initWithZero = false, int scale1 = 1,
      int scale2 = 1);
 ...
 public:
   ...
   */
  std::vector<Matrix> allocateMatrices(
   ...
   * @param initWithZero If true, initialize to zeros; else random values
   */
  void allocateMatrices(std::vector<Matrix>& mats,
                        bool shouldInitialize = false,
                        bool initWithZero = false);

  void allocateMatrices(std::vector<Matrix>& r_mats,
   ...
   */
  void freeAll(const std::vector<Matrix>& aMats,
