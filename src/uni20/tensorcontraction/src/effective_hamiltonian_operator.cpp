...
void EffectiveHamiltonianOperator::write_term_structure(std::string const& path, std::uint64_t index) const
{
  ...
  double const wall_clock_s =
      std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch()).count();

  fmt::print(file,
             "{{\"kind\":\"rabc_matvec\",\"index\":{},\"policy\":\"structure\",\"device_count\":1,"
             "\"block_count\":{},\"term_count\":{},\"wall_clock_s\":{:.6f},\"gpu_mem_used_bytes\":{},"
             "\"gpu_mem_free_bytes\":{},\"gpu_mem_total_bytes\":{},\"input_layout\":{},\"output_layout\":{},"
             "\"terms\":[",
             index, block_count, terms.size(), wall_clock_s, mem_total - mem_free, mem_free, mem_total, layout, layout);
  for (std::size_t term_index = 0; term_index < terms.size(); ++term_index)
  {
    auto const& term = terms[term_index];
    if (term_index != 0)
