...
std::vector<Matrix> MatrixAllocator::initializeMatricesFromChunk(
    const std::vector<std::pair<int, int>> &dims, double *chunkPtr,
...
std::vector<Matrix> MatrixAllocator::allocateMatrices(
    const std::vector<std::pair<int, int>> &dims, bool shouldInitialize,
  ...
  return initializeMatricesFromChunk(dims, chunkPtr, shouldInitialize,
                                     initWithZero, scale1, scale2);
}

void MatrixAllocator::allocateMatrices(std::vector<Matrix> &mats,
...
void MatrixAllocator::freeAll(const std::vector<Matrix> &aMats,
...
void MatrixAllocator::allocateMatrices(std::vector<Matrix> &r_mats,
                                       std::vector<Matrix> &a_mats,
                                       std::vector<Matrix> &b_mats,
                                       std::vector<Matrix> &c_mats,
                                       std::vector<Matrix> &inter_mats,
                                       Swapper &swapper) {
  ...
  fprintf(stderr, "[ALLOCATION][SUMMARY] Node=%d  Total InterMat: %.2f GB\n",
          mpi_rank, totalInterSize / (1024.0 * 1024.0 * 1024.0));
  ...
  fprintf(stderr, "[ALLOCATION][SUMMARY] Node=%d  Grand Total: %.2f GB\n",
          mpi_rank,
          (totalASize + totalBSize + totalCSize + totalInterSize + totalRSize) /
  ...
  for (int i = 0; i < device_count; i++) {
    ...
    fprintf(stderr, "[ALLOCATION][SUMMARY] Node=%d  Device %d: %.2f GB\n",
            mpi_rank, i, allocatedSize / (1024.0 * 1024.0 * 1024.0));
  }
