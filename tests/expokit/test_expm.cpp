...
template <typename Scalar> using Matrix = EXPOKIT::Matrix<Scalar>;
...
template <typename Scalar> Matrix<Scalar> MakeZeroMatrix(std::size_t order) { return Matrix<Scalar>(order, order); }
