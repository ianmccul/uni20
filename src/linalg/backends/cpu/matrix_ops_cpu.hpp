...
namespace util
{
    ...
    constexpr void require_rank_two(TensorView<T, Traits> const&)
    {
        static_assert(TensorView<T, Traits>::rank() == 2, "TensorView must model a rank-2 matrix");
    }

    template <typename T, typename Traits>
    void require_square(TensorView<T, Traits> view)
    {
    ...
    template <typename T, typename Traits>
    auto mutable_span(TensorView<T, Traits> view)
    {
        return view.mutable_mdspan();
    }

    template <typename T, typename Traits>
    auto const_span(TensorView<T, Traits> view)
    {
        return view.mdspan();
    }
}
...
template <typename TLhs, typename LhsTraits, typename TRhs, typename RhsTraits, typename TOut, typename OutTraits>
void add(TensorView<TLhs const, LhsTraits> lhs, TensorView<TRhs const, RhsTraits> rhs,
    TensorView<TOut, OutTraits> out)
{
...
namespace uni20::linalg
{
    ...
    template <typename T, typename Traits>
    void fill_identity(TensorView<T, Traits> out, cpu_tag)
    {
    ...
    template <typename TMat, typename MatTraits, typename Scalar, typename TOut, typename OutTraits>
    void scale_into(TensorView<TMat const, MatTraits> mat, Scalar const& scalar,
        TensorView<TOut, OutTraits> out, cpu_tag)
    {
    ...
    template <typename T, typename Traits>
    double matrix_one_norm(TensorView<T const, Traits> mat, cpu_tag)
    {
