...
| Dense singular value decomposition | yes | yes | yes | yes | Dispatched exact Tensor SVD through LAPACK `gesvd`, with singular-values-only, left-only, right-only, and complete APIs. Preserving and consuming synchronous/Async forms support independent reduced/full factor extents; reduced consuming factors may adopt the input allocation through LAPACK overwrite jobs. Complex singular values use the underlying real scalar type. |
