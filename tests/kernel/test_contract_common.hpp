...
template<typename T>
void matmul_col_major(int M, int K, int N,T alpha,T beta,T* A,T* B,T* C) {
    for (int i = 0; i < M; i++) 
        for (int j = 0; j < N; j++) 
            for (int k = 0; k < K; k++) 
...
template<typename T>
void matmul_row_major(int M, int K, int N,T alpha,T beta,T* A, T* B, T* C) {
    for (int i = 0; i < M; ++i) 
        for (int j = 0; j < N; ++j) 
            for (int k = 0; k < K; ++k)
...
template<typename Scalar, typename Tag>
void test_colunm_major_matmul_correctness() {
  typedef Scalar INPUTTYPE;
  std::cout<<"TEST disdhh 4 dim matirx gemv:"<<std::endl;
  std::random_device rd;                
  std::mt19937 gen(rd());               
  ...
  for (size_t i = 0; i < va.size(); ++i)  va[i] = fdis(gen);
  for (size_t i = 0; i < vb.size(); ++i)  vb[i] = fdis(gen);
  for (size_t i = 0; i < vc.size(); ++i)  vc[i] = fdis(gen);
  for (size_t i = 0; i < v_cpu.size(); ++i)  v_cpu[i] = vc[i];
  for (size_t i = 0; i < v_blas.size(); ++i)  v_blas[i] = vc[i];
  ...
  for(size_t i=0;i<arrangeA.size();i++) extentA.push_back(hash_table[arrangeA[i]]);
  for(size_t i=0;i<arrangeB.size();i++) extentB.push_back(hash_table[arrangeB[i]]);
  for(size_t i=0;i<arrangeC.size();i++) extentC.push_back(hash_table[arrangeC[i]]);
  std::array<size_t,rankA> extentA_array;
  std::array<size_t,rankB> extentB_array;
  std::array<size_t,rankC> extentC_array;
  ...
  for(size_t i=0; i<dimC;i++) standard_blas = standard_blas + (C_blas.data_handle()[i]-vc.data()[i])*(C_blas.data_handle()[i]-vc.data()[i]);
  for(size_t i=0; i<dimC;i++) standard_cpu = standard_cpu + (vc.data()[i]-C_cpu.data_handle()[i])*(vc.data()[i]-C_cpu.data_handle()[i]);
  std::cout<<"standard cpu-matmul-col-majpr: "<<standard_cpu<<"   "<<"standard blas-matmul-col-majpr: "<<standard_blas<<std::endl;
  ...
  for(size_t i=0;i<arrangeA.size();i++) strideA_array_row[i] = static_cast<ptrdiff_t>(strideA_row[i]);
  for(size_t i=0;i<arrangeB.size();i++) strideB_array_row[i] = static_cast<ptrdiff_t>(strideB_row[i]);
  for(size_t i=0;i<arrangeC.size();i++) strideC_array_row[i] = static_cast<ptrdiff_t>(strideC_row[i]);
  ...
  for(size_t i=0; i<dimC;i++) standard_blas = standard_blas +
   (C_blas_row.data_handle()[i]-vc.data()[i])*(C_blas_row.data_handle()[i]-vc.data()[i]);
  for(size_t i=0; i<dimC;i++) standard_cpu = standard_cpu + 
  (vc.data()[i]-C_cpu_row.data_handle()[i])*(vc.data()[i]-C_cpu_row.data_handle()[i]);
  std::cout<<"standard cpu-matmul-row-majpr: "<<standard_cpu<<"   "<<"standard blas-matmul-row-majpr: "<<standard_blas<<std::endl;
  ...
  for (size_t i = 0; i < 24; ++i)  va_rearrange[i] = i;


  std::cout<<"unrearrange:"<<std::endl;
  for (size_t i = 0 ;i < va_rearrange.size();i++)  std::cout<<va_rearrange.data()[i]<<" ";
  std::cout<<"\nrearrange 1:"<<std::endl;
  rearrange(va_rearrange.data(),vb_rearrange.data(),
            old_extent_rearrange,new_extent_rearrange,
            old_stride_rearrange,new_stride_rearrange);
  for (size_t i = 0 ;i < vb_rearrange.size();i++)  std::cout<<vb_rearrange.data()[i]<<" ";
  std::cout<<"\nrearrange 2:"<<std::endl;
  rearrange(vb_rearrange.data(),vc_rearrange.data(),
            new_extent_rearrange,old_extent_rearrange,
            new_stride_rearrange,old_stride_rearrange);
  for (size_t i = 0 ;i < vc_rearrange.size();i++)  std::cout<<vc_rearrange.data()[i]<<" ";
