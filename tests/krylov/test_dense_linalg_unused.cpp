...
uni20::DenseMatrix<Scalar> zero_matrix(Rows rows, Columns columns)
{
  uni20::DenseMatrix<Scalar> result(rows, columns);
...
template <typename Scalar>
uni20::DenseMatrix<Scalar> multiply_for_test(uni20::DenseMatrix<Scalar> const& lhs,
                                             uni20::DenseMatrix<Scalar> const& rhs)
{
  ...
  auto result = zero_matrix<Scalar>(lhs.rows(), rhs.cols());
...
template <typename Scalar> uni20::DenseMatrix<Scalar> transpose(uni20::DenseMatrix<Scalar> const& matrix)
{
  auto result = zero_matrix<Scalar>(matrix.cols(), matrix.rows());
...
template <typename Scalar> uni20::DenseMatrix<Scalar> identity_matrix(std::size_t order)
{
  auto result = zero_matrix<Scalar>(order, order);
...
template <typename Scalar> uni20::DenseMatrix<Scalar> conjugate_transpose(uni20::DenseMatrix<Scalar> const& matrix)
{
  auto result = zero_matrix<Scalar>(matrix.cols(), matrix.rows());
  for (uni20::index_type row = 0; row < matrix.rows(); ++row)
  {
...
template <typename Scalar>
void expect_reconstructs(uni20::DenseMatrix<Scalar> const& original, uni20::DenseMatrix<Scalar> const& schur_form,
                         uni20::DenseMatrix<Scalar> const& schur_vectors)
{
  ...
  {
    for (uni20::index_type col = 0; col < original.cols(); ++col)
    {
...
template <typename Scalar>
void expect_complex_reconstructs(uni20::DenseMatrix<Scalar> const& original,
                                 uni20::DenseMatrix<Scalar> const& schur_form,
                                 uni20::DenseMatrix<Scalar> const& schur_vectors)
{
  auto reconstructed =
...
void expect_generalized_schur_reconstructs(
    uni20::DenseMatrix<Scalar> const& original_matrix, uni20::DenseMatrix<Scalar> const& original_metric,
    uni20::krylov::RealGeneralizedSchurDecomposition<Scalar> const& decomposition)
{
  ...
  auto reconstructed_metric =
      multiply_for_test(multiply_for_test(decomposition.left_schur_vectors, decomposition.metric_schur_form),
                        transpose(decomposition.right_schur_vectors));
  for (uni20::index_type row = 0; row < original_matrix.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < original_matrix.cols(); ++col)
    {
...
template <typename Scalar>
Scalar
schur_right_eigenvector_residual_max(uni20::DenseMatrix<Scalar> const& schur_form, uni20::complex<Scalar> eigenvalue,
                                     uni20::DenseMatrix<uni20::complex<Scalar>> const& eigenvectors, std::size_t column)
{
  Scalar residual_max{};
...
template <typename Scalar>
Scalar generalized_schur_right_eigenvector_residual_max(uni20::DenseMatrix<Scalar> const& matrix_schur_form,
                                                        uni20::DenseMatrix<Scalar> const& metric_schur_form,
                                                        uni20::complex<Scalar> eigenvalue,
                                                        uni20::DenseMatrix<uni20::complex<Scalar>> const& eigenvectors,
                                                        std::size_t column)
{
  Scalar residual_max{};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, StoresMatrixColumnMajor)
{
  ...
  auto matrix = zero_matrix<Scalar>(2, 3);
  matrix[0, 0] = Scalar{1};
  matrix[0, 1] = Scalar{2};
  matrix[0, 2] = Scalar{3};
  matrix[1, 0] = Scalar{4};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesDenseRealSymmetricMatrixNorms)
{
  ...
  auto matrix = zero_matrix<Scalar>(2, 2);
  matrix[0, 0] = Scalar{2};
  matrix[0, 1] = Scalar{-3};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, RefinesDenseRealLinearSystemWithDiagnostics)
{
  ...
  auto expected = zero_matrix<Scalar>(2, 1);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesFromDenseRealLuFactorization)
{
  ...
  auto transposed_expected = zero_matrix<Scalar>(2, 1);
  ...
  for (uni20::index_type row = 0; row < transposed_solution.rows(); ++row)
  {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, InvertsDenseRealTriangularMatrix)
{
  ...
  for (uni20::index_type row = 0; row < product.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < product.cols(); ++col)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesDenseRealSylvesterEquation)
{
  ...
  auto left = zero_matrix<Scalar>(1, 1);
  left[0, 0] = Scalar{2};

  auto right = zero_matrix<Scalar>(1, 1);
  right[0, 0] = Scalar{3};

  auto rhs = zero_matrix<Scalar>(1, 1);
  rhs[0, 0] = Scalar{10};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesDenseRealLeastSquaresProblem)
{
  ...
  auto coefficients = zero_matrix<Scalar>(3, 2);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesDenseRankRevealingLeastSquaresProblem)
{
  ...
  auto expected_one_solution = zero_matrix<Scalar>(2, 1);
  ...
  for (uni20::index_type row = 0; row < rhs.rows(); ++row)
  {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesDensePivotedCholeskyFactorization)
{
  ...
  auto upper = zero_matrix<Scalar>(2, 2);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesSelectedDenseRealSymmetricEigenvectors)
{
  ...
  auto matrix = zero_matrix<Scalar>(3, 3);
  matrix[0, 0] = Scalar{1};
  matrix[1, 1] = Scalar{2};
  matrix[2, 2] = Scalar{3};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesDenseGeneralizedRealSymmetricEigenvectors)
{
  ...
  auto metric = zero_matrix<Scalar>(2, 2);
  metric[0, 0] = Scalar{1};
  metric[1, 1] = Scalar{3};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesSelectedDenseGeneralizedRealSymmetricEigenvectors)
{
  ...
  auto metric = zero_matrix<Scalar>(3, 3);
  metric[0, 0] = Scalar{1};
  metric[1, 1] = Scalar{2};
  metric[2, 2] = Scalar{3};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesReducedRealQrFactorization)
{
  ...
  auto matrix = zero_matrix<Scalar>(3, 2);
  matrix[0, 0] = Scalar{1};
  matrix[1, 0] = Scalar{1};
  matrix[2, 0] = Scalar{};
  matrix[0, 1] = Scalar{1};
  ...
  for (uni20::index_type row = 0; row < gram.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < gram.cols(); ++col)
    {
      Scalar const expected = row == col ? Scalar{1} : Scalar{};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealBidiagonalReductionTallMatrix)
{
  ...
  auto matrix = zero_matrix<Scalar>(4, 3);
  matrix[0, 0] = Scalar{1};
  matrix[1, 0] = Scalar{2};
  matrix[2, 0] = Scalar{-1};
  matrix[3, 0] = Scalar{4};
  ...
  for (uni20::index_type row = 0; row < reduction.bidiagonal.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < reduction.bidiagonal.cols(); ++col)
    {
  ...
  for (uni20::index_type row = 0; row < q_gram.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < q_gram.cols(); ++col)
    {
      Scalar const expected = row == col ? Scalar{1} : Scalar{};
  ...
  for (uni20::index_type row = 0; row < p_gram.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < p_gram.cols(); ++col)
    {
      Scalar const expected = row == col ? Scalar{1} : Scalar{};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealBidiagonalReductionWideMatrix)
{
  ...
  auto matrix = zero_matrix<Scalar>(3, 4);
  matrix[0, 0] = Scalar{1};
  matrix[1, 0] = Scalar{2};
  matrix[2, 0] = Scalar{-1};
  matrix[0, 1] = Scalar{3};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealBidiagonalDivideAndConquerSvd)
{
  ...
  auto sigma = zero_matrix<Scalar>(3, 3);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesSelectedRealBidiagonalSvd)
{
  ...
  auto sigma = zero_matrix<Scalar>(2, 2);
  for (std::size_t index = 0; index < result.singular_values.size(); ++index)
  {
    sigma[index, index] = result.singular_values[index];
  }
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealHessenbergReductionAndOrthogonalFactor)
{
  ...
  auto matrix = zero_matrix<Scalar>(4, 4);
  matrix[0, 0] = Scalar{4};
  matrix[1, 0] = Scalar{3};
  matrix[2, 0] = Scalar{-2};
  matrix[3, 0] = Scalar{1};
  ...
  for (uni20::index_type col = 0; col < reduction.hessenberg.cols(); ++col)
  {
    for (uni20::index_type row = col + 2; row < reduction.hessenberg.rows(); ++row)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, AppliesRealHessenbergOrthogonalFactor)
{
  ...
  auto left_target = zero_matrix<Scalar>(4, 2);
  ...
  auto right_target = zero_matrix<Scalar>(2, 4);
  ...
  for (uni20::index_type row = 0; row < left_applied.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < left_applied.cols(); ++col)
    {
  ...
  for (uni20::index_type row = 0; row < right_applied.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < right_applied.cols(); ++col)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealSymmetricTridiagonalReductionAndOrthogonalFactor)
{
  ...
  for (uni20::index_type col = 0; col < reduction.tridiagonal.cols(); ++col)
  {
    for (uni20::index_type row = 0; row < reduction.tridiagonal.rows(); ++row)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, AppliesCompactRealQrFactorization)
{
  ...
  for (uni20::index_type row = 0; row < left_q.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < left_q.cols(); ++col)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, AppliesCompactRealQlFactorization)
{
  ...
  auto recovered_identity = uni20::krylov::apply_real_ql_factor(compact, left_q, uni20::krylov::MatrixSide::Left,
                                                                uni20::krylov::MatrixTranspose::Transpose);
  uni20::index_type const first_reduced_col = left_q.cols() - explicit_ql.q.cols();
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, AppliesCompactRealRqFactorization)
{
  ...
  auto recovered_identity = uni20::krylov::apply_real_rq_factor(compact, right_q, uni20::krylov::MatrixSide::Right,
                                                                uni20::krylov::MatrixTranspose::Transpose);
  uni20::index_type const first_reduced_row = right_q.rows() - explicit_rq.q.rows();

  ASSERT_EQ(compact.rank, 2);
  ASSERT_EQ(compact.tau.size(), 2);
  ASSERT_EQ(right_q.rows(), 3);
  ASSERT_EQ(right_q.cols(), 3);
  for (uni20::index_type row = 0; row < right_q.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < right_q.cols(); ++col)
    {
      EXPECT_NEAR(static_cast<double>((left_q[row, col])), static_cast<double>((right_q[row, col])),
                  scaled_tolerance<Scalar>(100.0));
      EXPECT_NEAR(static_cast<double>((recovered_identity[row, col])), static_cast<double>((identity[row, col])),
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, AppliesRealBidiagonalOrthogonalFactors)
{
  ...
  {
    for (uni20::index_type col = 0; col < q.cols(); ++col)
    {
  ...
  for (uni20::index_type row = 0; row < pt.rows(); ++row)
  {
    for (uni20::index_type col = 0; col < pt.cols(); ++col)
    {
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealHessenbergSchurDecomposition)
{
  ...
  auto hessenberg = zero_matrix<Scalar>(3, 3);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesLayoutRightSchurForLogicalMatrix)
{
  ...
  uni20::DenseMatrix<Scalar> left_matrix = uni20::krylov::copy_right_to_left(matrix);
  uni20::DenseMatrix<Scalar> left_schur_form = uni20::krylov::copy_right_to_left(result.schur_form);
  uni20::DenseMatrix<Scalar> left_schur_vectors = uni20::krylov::copy_right_to_left(result.schur_vectors);
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealSchurRightEigenvectors)
{
  ...
  for (uni20::index_type column = 0; column < result.right_eigenvectors.cols(); ++column)
  {
    Scalar vector_max{};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, SolvesRealNonsymmetricDiagonalEigenvalues)
{
  ...
  auto matrix = zero_matrix<Real>(3, 3);
  matrix[0, 0] = Real{3};
  matrix[1, 1] = Real{-1};
  matrix[2, 2] = Real{2};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, BalancesRealNonsymmetricMatrixAndBacktransformsRightVectors)
{
  ...
  auto matrix = zero_matrix<Real>(2, 2);
  matrix[0, 0] = Real{};
  ...
  auto vectors = zero_matrix<Real>(2, 2);
  vectors[0, 0] = Real{1};
  vectors[1, 1] = Real{1};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, BalancesRealGeneralizedNonsymmetricPencilAndBacktransformsRightVectors)
{
  ...
  auto metric = zero_matrix<Real>(2, 2);
  metric[0, 0] = Real{1};
  metric[1, 1] = Real{2};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealGeneralizedHessenbergReduction)
{
  ...
  auto metric = zero_matrix<Real>(3, 3);
  metric[0, 0] = Real{2};
  metric[0, 1] = Real{-1};
  metric[1, 1] = Real{3};
  metric[0, 2] = Real{1};
...
TYPED_TEST(KrylovDenseSubspaceTypedTest, ComputesRealGeneralizedHessenbergSchurDecomposition)
{
  ...
  auto hessenberg = zero_matrix<Real>(3, 3);
