...
- `docs/cuda_runtime_design_notes.md` — stream ownership and the idle-stream pool.
- `docs/execution_architecture.md` — movement/allocation overhead as the real lever.
- `docs/block_coalescing.md` — the single-buffer packing that doubles as allocation.
- `docs/tensorcontraction_integration_findings.md` — the execution profile that motivated this.
...
(`block_coalescing.md`): packing the tail's many tiny blocks into one strided
...
- Default release threshold and prime size per device class, and how they interact
  with the stream-pool backpressure in `cuda_runtime_design_notes.md`.
...
- Allocation lifetime under deferred (buffer-access) sync: the
  token-pins-storage rule (`ordering_and_backend_lowering.md`) must drive
