...
works for arbitrary strided tensors that satisfy the texttt{StridedMdspan} concept, even if they are not in a
...
void rearrange(extents_type Extents, stride_type InputStrides, stride_type OutputStrides, T const* In, T* Out)
  \end{lstlisting}
  We could re-use the `extent_strides` structure, which means we can use the existing functions to merge indices if possible.
  We probably don't want to use `extent_strides` in the API interface though, since we won't naturally have the strides
