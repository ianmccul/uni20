...
class Swapper {
  ...
  using preStoreMapType =
      std::unordered_map<int, std::pair<int, std::shared_ptr<GpuBuffer>>>;
 ...
 public:
  ...
  std::shared_ptr<GpuBuffer> getForRead(Matrix mat, int deviceId,
                                        cudaStream_t stream);
  std::shared_ptr<GpuBuffer> getForWrite(Matrix mat, int deviceId,
                                         cudaStream_t stream);
  ...
  std::pair<int, std::shared_ptr<GpuBuffer>> getPreStoreBufferOrNone(
      Matrix mat);
  void copyMatrix(Matrix mat, std::shared_ptr<GpuBuffer> buffer, int deviceId,
                  cudaStream_t stream, StreamManager &streamManager);
