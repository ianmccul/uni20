...
template <typename T> Matrix<T> make_identity(std::size_t n)
{
  Matrix<T> result(n, n);
...
template <typename T> Matrix<T> multiply(Matrix<T> const& lhs, Matrix<T> const& rhs)
{
...
template <typename T> Matrix<T> add(Matrix<T> const& lhs, Matrix<T> const& rhs)
{
...
template <typename T> Matrix<T> subtract(Matrix<T> const& lhs, Matrix<T> const& rhs)
{
  if (lhs.rows() != rhs.rows() || lhs.cols() != rhs.cols())
  {
...
template <typename T, typename Scalar> Matrix<T> scale(Matrix<T> const& mat, Scalar const& scalar)
{
...
template <typename T> Matrix<T> matrix_power(Matrix<T> const& mat, unsigned int power)
{
...
template <typename T> void swap_rows(Matrix<T>& mat, std::size_t lhs, std::size_t rhs)
{
  if (lhs == rhs)
  {
    return;
