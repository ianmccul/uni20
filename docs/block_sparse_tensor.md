...
- `docs/block_coalescing.md` — single-axis GEMM grouping over the layout.
- `docs/execution_architecture.md` — how tensor, dispatch, and scheduling compose.
- `docs/qnum.md` — quantum numbers and symmetry.
- `docs/backend_dispatch.md` — kernel dispatch the tensor feeds.
...
policy, layered on top (see `execution_architecture.md`).
...
3. **Token-pins-storage lifetime.** Following the two-clocks rule in
   `ordering_and_backend_lowering.md`: buffer storage lifetime is owned by the
