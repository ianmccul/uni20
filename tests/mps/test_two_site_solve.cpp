...
TEST(TwoSiteSolveTest, FindsTwoSiteHeisenbergGroundState)
{
  ...
  EXPECT_EQ(result.optimized_vector.block(0), (tensorcontraction::MatrixFamily::Block{4, 1}));
...
TEST(TwoSiteSolveTest, ConvertsVectorBackToMatrixLayout)
{
  ...
  std::array vector_block{tensorcontraction::MatrixFamily::Block{4, 1}};
  tensorcontraction::MatrixFamily vector(vector_block);
  vector.assign(0, std::array{1.0, 2.0, 3.0, 4.0});
