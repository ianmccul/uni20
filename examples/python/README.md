# Python Example

`python_buildinfo_example.py` imports the built `uni20` module and prints its
formatted build information. Pass the bindings directory as the first argument
or configure `PYTHONPATH` first:

```bash
python3 examples/python/python_buildinfo_example.py build/bindings/python
```

See the [examples index](../), [Python Bindings](../../docs/python/bindings.md),
and [Build Information](../../docs/development/build_information.md).
