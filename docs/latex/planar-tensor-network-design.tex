...
\newcommand{\CoSpace}{\mathsf{CoSpace}}
...
network geometry, and contraction plans.  Mutable labels may remain as
display metadata or as local notation inside a contraction expression,
but they cannot be the canonical identity of tensor legs.
\end{abstract}
...
\begin{itemize}[nosep]
  ...
  \item labels are useful for display and diagnostics, but they should
        not be the authority on mathematical identity.
\end{itemize}
...
\begin{itemize}
  ...
  \item optional display labels attached to spaces or legs;
...
\begin{itemize}[nosep]
  ...
  \item Is this leg a \(\Space\) or \(\CoSpace\)?
...
\subsection{Space Identity Invariant}
...
They should not be interchangeable unless the library has explicit
evidence that they are the same space.

This is where labels are useful if they label spaces, not tensor-leg
positions.  A finite MPS can label its bond spaces
\[
  b_0,b_1,\ldots,b_L,
\]
and then contractions over the wrong bond fail even in the dense case.
...
\[
  S : B \to B.
\]
Thus \(S\) naturally has the same space label on its two sides.  A
global rule that tensor labels must be unique is incompatible with this
mathematical structure.

\subsection{Metadata Is Not Semantics}

Display labels, names, colors, and diagram coordinates help humans.
They are not the authority on which operations are legal.

Changing a display label should never alter contraction legality.
Changing a space identity or applying a braid should be a separate,
checked operation.
...
\midrule
\texttt{relabel} & rename an axis & display metadata only \\
...
\begin{description}
  \item[\texttt{relabel}] Change display names only.
...
\begin{verbatim}
phys = Space("SpinHalf", symmetry=SU2, sectors=[j(1,2)])
b0   = Space("bond-0", symmetry=SU2, sectors=[j(0)])
b1   = Space("bond-1", symmetry=SU2, sectors=[j(0), j(1)])
...
\begin{itemize}[nosep]
  ...
  \item space identity or allowed isomorphism;
...
dimension but different identities, \texttt{bond-4} vs \texttt{bond-5}.
\end{quote}
...
\subsection{Dense Space Identity Tests}

Even with no symmetry, create two spaces with the same dimension but
...
two sides of \(S\) carry the same space identity and that \(U S V\)
reconstructs the original tensor with the expected boundary.

\subsection{Relabel Non-Effect Tests}

Changing display labels must not change which contractions are legal.
If a contraction fails before relabelling, and no space identity,
duality, braid, or recoupling operation has been applied, it should
still fail after relabelling.
...
\begin{itemize}
  \item Persistent labels can be read as display labels.
  ...
  \item Relabelling should not make a previously illegal contraction
        legal.
...
global planar embedding.  Any interface that lets users repair
contractions by mutating labels has already lost the essential
correctness property.

The practical design rule is:

\begin{quote}
Use labels for notation and diagnostics.  Use spaces, duals, ordered
legs, fusion trees, and planar diagrams for semantics.
\end{quote}
