...
#define DEBUG_GPU_COPY_H2D(swapper, matId, deviceId, stream, size)            \
  do {                                                                        \
    ...
    fprintf(                                                                  \
        stderr,                                                               \
        "[SWAPPER][H2D_COPY] Matrix=%s Device=%d Stream=%p Size=%zu bytes\n", \
        name.c_str(), deviceId, (void *)stream, size);                        \
  } while (0)
...
#define DEBUG_GPU_COPY_D2H(swapper, matId, deviceId, stream, size)            \
  do {                                                                        \
    ...
    fprintf(                                                                  \
        stderr,                                                               \
        "[SWAPPER][D2H_COPY] Matrix=%s Device=%d Stream=%p Size=%zu bytes\n", \
        name.c_str(), deviceId, (void *)stream, size);                        \
  } while (0)

#define DEBUG_GPU_COPY_D2D(swapper, matId, srcDeviceId, src, dstDeviceId, dst, \
                           stream, size)                                       \
  do {                                                                         \
    std::string name = (swapper).getMatrixName(matId);                         \
    fprintf(stderr,                                                            \
            ...
            name.c_str(), srcDeviceId, (void *)src, dstDeviceId, (void *)dst,  \
            (void *)stream, size);                                             \
  } while (0)
...
#define DEBUG_GPU_ALLOC(swapper, matId, deviceId, stream, size)                \
  do {                                                                         \
    ...
    fprintf(                                                                   \
        stderr,                                                                \
        "[SWAPPER][GPU_ALLOC] Matrix=%s Device=%d Stream=%p Size=%zu bytes\n", \
        name.c_str(), deviceId, (void *)stream, size);                         \
  } while (0)
...
#define DEBUG_GPU_EVICT(swapper, matId, deviceId, stream, reason)             \
  do {                                                                        \
    std::string name = (swapper).getMatrixName(matId);                        \
    fprintf(stderr,                                                           \
            "[SWAPPER][GPU_EVICT] Matrix=%s Device=%d Stream=%p Reason=%s\n", \
            name.c_str(), deviceId, (void *)stream, reason);                  \
  } while (0)
...
#define DEBUG_BUFFER_REUSE(swapper, matId, deviceId, stream, forWrite)      \
  do {                                                                      \
    std::string name = (swapper).getMatrixName(matId);                      \
    fprintf(stderr,                                                         \
            "[SWAPPER][REUSE] Matrix=%s Device=%d Stream=%p ForWrite=%d\n", \
            name.c_str(), deviceId, (void *)stream, forWrite);              \
  } while (0)
...
#define DEBUG_MATMUL(swapper, result, m1, m2, alpha, deviceId, stream)       \
  do {                                                                       \
    ...
    fprintf(                                                                 \
        stderr,                                                              \
        "[CALCULATOR][MATMUL] %s = %s * %s * %.2lf (Device=%d Stream=%p)\n", \
...
#define DEBUG_ADD_ACCU(swapper, result, m1, alpha, deviceId, stream)          \
  do {                                                                        \
    ...
    std::string a = (swapper).getMatrixName(m1);                              \
    fprintf(stderr,                                                           \
            "[CALCULATOR][ADD_ACCU] %s += %s * %.2f (Device=%d Stream=%p)\n", \
            r.c_str(), a.c_str(), alpha, deviceId, (void *)stream);           \
  } while (0)
...
#define DEBUG_TERM_START(swapper, termIdx, rIdx, aIdx, bIdx, cIdx, fval,       \
                         deviceId)                                             \
  do {                                                                         \
    ...
    fprintf(stderr,                                                            \
            "[ARRANGER][TERM_%d] R=%s A=%s B=%s C=%s fval=%.2f (Device=%d)\n", \
            termIdx, r.c_str(), a.c_str(), b.c_str(), c.c_str(), fval,         \
            deviceId);                                                         \
  } while (0)
...
#define DEBUG_SIGNAL_WORK(swapper, matId, deviceId, currentValue)           \
  do {                                                                      \
    std::string name = (swapper).getMatrixName(matId);                      \
    fprintf(stderr,                                                         \
            "[CALCULATOR][SIGNAL] Matrix=%s Device=%d Counter: %d -> %d\n", \
            name.c_str(), deviceId, currentValue, currentValue + 1);        \
  } while (0)
...
#define DEBUG_NCCL_REDUCE(swapper, matId, root, deviceId, id, count, comm, \
                          stream)                                          \
  do {                                                                     \
    std::string name = (swapper).getMatrixName(matId);                     \
    fprintf(stderr,                                                        \
            ...
            name.c_str(), root, deviceId, id, count, (void *)comm,         \
            (void *)stream);                                               \
  } while (0)
...
#define DEBUG_NCCL_COMM_CREATE_START(NodeID, numRanks, idPtr)                \
  do {                                                                       \
    ...
    fprintf(stderr,                                                          \
            ...
            "[%02x%02x"                                                      \
            "%02x%02x...]\n",                                                \
            NodeID, numRanks, (void *)idPtr, bytes[0], bytes[1], bytes[2],   \
            bytes[3]);                                                       \
  } while (0)
...
#define DEBUG_NCCL_COMM_INIT(NodeId, deviceId, rank, numRanks, commPtr, idPtr) \
  do {                                                                         \
    ...
    fprintf(stderr,                                                            \
            ...
            NodeId, deviceId, rank, numRanks, (void *)commPtr, (void *)idPtr,  \
...
#define DEBUG_MEMSET(swapper, matId, deviceId, size, stream)                 \
  do {                                                                       \
    std::string name = (swapper).getMatrixName(matId);                       \
    fprintf(stderr,                                                          \
            "[CALCULATOR][MEMSET] Matrix=%s Device=%d Size=%zu Stream=%p\n", \
            name.c_str(), deviceId, size, (void *)stream);                   \
  } while (0)
...
#define DEBUG_MEM_INFO(deviceId, freeMem, totalMem)                        \
  do {                                                                     \
    fprintf(stderr, "[MEMORY][INFO] Device=%d Free=%.2fGB Total=%.2fGB\n", \
            deviceId, (freeMem) / (1024.0 * 1024.0 * 1024.0),              \
...
#define DEBUG_MATRIX_DISTRIBUTION(swapper, rank, mat_id)                     \
  do {                                                                       \
    fprintf(stderr, "[ARRANGER][MPI] Node=%d get matrix %s (ID=%d)\n", rank, \
            (swapper).getMatrixName(mat_id).c_str(), mat_id);                \
  } while (0)
...
#define DEBUG_HIT(swapper, mat_id, device_id, size)                       \
  do {                                                                    \
    fprintf(stderr, "[SWAPPER][HIT] Device=%d Matrix %s hit, Size=%zu\n", \
            device_id, (swapper).getMatrixName(mat_id).c_str(), size);    \
  } while (0)
#define DEBUG_SECOND_HIT(swapper, mat_id, src_device_id, target_device_id,   \
                         size)                                               \
  do {                                                                       \
    fprintf(stderr,                                                          \
            "[SWAPPER][SECOND_HIT] DeviceId=%d Matrix %s hit on Device=%d, " \
            "Size=%zu\n",                                                    \
            target_device_id, (swapper).getMatrixName(mat_id).c_str(),       \
            src_device_id, size);                                            \
  } while (0)
#define DEBUG_THIRD_HIT(swapper, mat_id, src_device_id, target_device_id,      \
                        size)                                                  \
  do {                                                                         \
    fprintf(stderr,                                                            \
            "[SWAPPER][THIRD_HIT] DeviceId=%d Matrix %s hit on Device=%d, "    \
            "Size=%zu\n",                                                      \
            target_device_id, (swapper).getMatrixName(mat_id).c_str(), src_device_id, \
            size);                                                             \
  } while (0)
