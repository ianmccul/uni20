...
\paragraph{Status.}
This November 2025 paper is algorithmic background.  Uni20's current operation boundary selects a
backend from policy-bearing Tensor views, normalizes fixed operands to \texttt{mdspec} values, and
lets the selected backend acquire execution-domain mdspans.  Implemented backends include an
accessor-respecting CPU reference loop, direct no-copy GEMM lowering when the M, N, and K stride
groups each collapse to one dimension, and looped GEMM when exactly one residual M or N descriptor
remains.  Multiple residual dimensions, residual-K accumulation, and pack--GEMM--unpack remain
backend strategies rather than Tensor-front-end rewrites.  The canonical current contract is
\texttt{docs/linalg/tensor\_contraction.md}.
...
\texttt{stdex} namespace, as \texttt{stdex::mdspan}. Uni20 source includes the
configured gateway \texttt{uni20/mdspan/mdspan.hpp}; that header validates the
generated namespace configuration before including the external
\texttt{mdspan/mdspan.hpp} implementation header.
...
\texttt{src/uni20/mdspan/mdspan.hpp} and
\texttt{src/uni20/mdspan/concepts.hpp}). Any type that models the
\texttt{StridedMdspanLike} concept exposes the rank, extents, and per-index strides
required to navigate strided storage. Leaf kernels may therefore accept plain
\texttt{mdspan} and custom resolved views directly. Tensor wrappers remain a
higher-level abstraction and resolve their \texttt{mdspan()} only at the
dispatch boundary.
...
\texttt{mdspan()} and use \texttt{data\_handle()}, extents, and strides to drive the computation.
...
An extent-one descriptor is an identity for merging. Its only logical index is zero, so its
reported strides are never observed: it may merge with either neighbour regardless of those stride
values, and the merged descriptor retains the non-singleton neighbour's strides. Two singleton
descriptors merge to canonical unit strides. Extent-zero descriptors remain distinct because they
represent an empty iteration space rather than an unobserved factor of one.
...
works for arbitrary strided tensors that satisfy the \texttt{StridedMdspanLike} concept, even if they are not in a
...
\begin{description}
  \item[Looped GEMM.]  If only one group fails to merge completely, we can treat the remaining
        ...
        slice.  This preserves GEMM's efficiency while avoiding a full rearrangement.  The current
        first implementation supports one residual $M$ or $N$ descriptor.  It advances immediate
        handles through their accessor or deferred descriptors through an offset operation before
        ordinary GEMM dispatch.  Residual $K$ loops remain future work because they require
        first-slice versus accumulating-$\beta$ semantics.
