...
- **Tiny kernels** where event overhead exceeds the kernel may be *faster* blocking
  ...
  one" as a performance decision: event recording, stream waits, and event tracing
  are not free, and can dominate for event-heavy small-kernel workloads.
...
- Where does the placement/cost policy (which device or rank a block lives on) sit
  ...
  ordering layer must remain correct for any placement the policy chooses. A
  block/matrix-level placement-and-cost model is being explored in prototype form.
