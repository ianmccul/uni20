...
- `docs/execution_architecture.md` — mechanism vs policy; the planner owns placement.
- `docs/backend_dispatch.md` — `maybe_can_*` / `try_*` / generic kernel dispatch.
- `docs/qnum.md` — `Symmetry`, `QNum`, `QNumList`, `BlockSpace` (the symmetry foundation this builds on).
- `docs/kernel_dispatch.md` — the `backend_list` walk and the async/scheduler seam this tensor feeds.
- `docs/uni20_mpi_persistent_dispatch_design.md` — persistent immutable objects; the replication model behind `ReplicatedBlockTensor`.
...
`storage_kind_and_location.md` symmetry guarantees.
...
device execution, per the two-clocks rule in `ordering_and_backend_lowering.md`).
...
- **`ReplicatedBlockTensor`** — implicitly **const**; its blocks are **not
  ...
  model (`uni20_mpi_persistent_dispatch_design.md`). It takes the **same**
...
facet in `kernel_dispatch.md`). The CUDA and MPI paths likewise decide for
...
scheduled async operation (`storage_kind_and_location.md`), never implicit. This is
...
(`ordering_and_backend_lowering.md`) — are the `LegList`, the
...
- **Hazard granularity** — buffer-granularity (conservative, simple) vs sub-range
  ...
  `kernel_dispatch.md` open question 7 (`AsyncArray` queue representation), seen from
...
- **`ReplicatedBlockTensor` commit semantics** — "move in a *finished* tensor" needs a
  ...
  `ordering_and_backend_lowering.md`). Is the commit an explicit
