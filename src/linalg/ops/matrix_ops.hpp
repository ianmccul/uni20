...
namespace detail
{
    ...
    using view_t = std::remove_cvref_t<View>;

    template <typename View>
    using value_type_t = std::remove_cv_t<typename view_t<View>::value_type>;

    template <typename View>
    using default_tag_t = typename view_t<View>::default_tag;

    template <typename FirstView, typename... RestViews>
    using select_tag_t =
        std::common_type_t<default_tag_t<FirstView>, default_tag_t<RestViews>...>;

    template <typename FirstView, typename... RestViews>
    constexpr auto select_tag(FirstView const&, RestViews const&...)
        -> select_tag_t<FirstView, RestViews...>
    {
    ...
    void require_same_shape(LhsView const& lhs, RhsView const& rhs)
    {
    ...
    auto clone(TensorView<T const, Traits> view, BackendTag tag)
    {
        using value_type = value_type_t<decltype(view)>;
        using tensor_type =
            uni20::BasicTensor<value_type, stdex::dextents<index_type, 2>, VectorStorage>;
...
void fill_identity(TensorView<T, Traits> out)
{
...
void multiply_into(TensorView<T const, LhsTraits> lhs, TensorView<T const, RhsTraits> rhs,
...
template <typename T, typename LhsTraits, typename RhsTraits, typename OutTraits>
void add_into(TensorView<T const, LhsTraits> lhs, TensorView<T const, RhsTraits> rhs,
    TensorView<T, OutTraits> out)
{
    auto const tag = detail::select_tag(lhs, rhs, out);
...
template <typename T, typename LhsTraits, typename RhsTraits, typename OutTraits>
void subtract_into(TensorView<T const, LhsTraits> lhs, TensorView<T const, RhsTraits> rhs,
    TensorView<T, OutTraits> out)
{
    auto const tag = detail::select_tag(lhs, rhs, out);
...
void scale_into(TensorView<T const, MatTraits> mat, Scalar const& scalar,
    TensorView<T, OutTraits> out)
{
...
template <typename T, typename Traits>
double matrix_one_norm(TensorView<T const, Traits> mat)
{
    auto const tag = detail::select_tag(mat);
...
void matrix_power_into(TensorView<T const, MatTraits> mat, unsigned int power,
...
auto matrix_power(TensorView<T const, MatTraits> mat, unsigned int power)
{
    auto const tag = detail::select_tag(mat);
...
template <typename T, typename Traits>
void swap_rows(TensorView<T, Traits> mat, index_type lhs, index_type rhs)
{
    auto const tag = detail::select_tag(mat);
...
auto make_identity(index_type order, BackendTag tag)
{
    using tensor_type =
        uni20::BasicTensor<T, stdex::dextents<index_type, 2>, VectorStorage>;
...
template <typename T>
auto make_identity(index_type order)
{
